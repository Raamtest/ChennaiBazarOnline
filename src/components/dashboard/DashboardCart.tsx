...
const DashboardCart = () => {
  ...
  const { data: cartItems = [], isLoading, isError } = useQuery({
    ...
    queryFn: async () => {
      ...
      // Combine cart items with their products
      return cartData.map(item => ({
        ...item,
        product: productsData.find(product => product.id === item.product_id) || null
      }));
    },
    enabled: !!user?.id,
  });
  ...
  if (isError) {
    return <div className="flex justify-center items-center h-64 text-red-500">Error loading cart data</div>;
  }
